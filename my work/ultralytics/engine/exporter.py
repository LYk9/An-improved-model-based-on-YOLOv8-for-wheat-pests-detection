...
class Exporter:
    ...
    def __call__(self, model=None):
        ...
        if any(f):
            ...
            s = (
                ""
                if square
                else f"WARNING ⚠️ non-PyTorch val-EMA requires square images, 'imgsz={self.imgsz}' will not "
                f"work. Use export 'imgsz={max(self.imgsz)}' if val-EMA is required."
            )
            ...
            LOGGER.info(
                ...
                f'\nValidate:        yolo val-EMA task={model.task} model={f} imgsz={imgsz} data={data} {q} {s}'
    ...
    def export_openvino(self, prefix=colorstr("OpenVINO:")):
        ...
        if self.args.int8:
            ...
            dataset = YOLODataset(data["val-EMA"], data=data, imgsz=self.imgsz[0], augment=False)
